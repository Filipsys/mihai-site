...
const ProcessCard = (props: {
  ...
  <div className="bg-background relative md:cursor-pointer p-4 h-full border-[1px] group border-white/5 flex gap-4 *:first:text-secondary *:first:h-fit md:items-center md:py-2 lg:items-start lg:cursor-auto">
    <props.icon />

    <div className="flex w-full h-full justify-between md:items-center lg:items-start">
      <div className="flex flex-col">
        <p className="text-text-secondary font-semibold">{props.title}</p>
        <p className="md:hidden lg:block">{props.description}</p>
      </div>

      <p className="italic text-base text-text-secondary/25">#{props.index}</p>
    </div>

    <div className="hidden cursor-auto text-base md:group-hover:block lg:group-hover:hidden absolute bg-primary z-10 p-2 px-4 border-[1px] border-white/5 bottom-0 left-0 translate-y-full w-[calc(100%+2px)] -translate-x-[1px]">
...
export const WorkProcess = (props: {
  ...
  <div
    ref={props.ref}
    className="bg-primary h-fit flex flex-col p-4 border-[1px] border-white/5"
  >
